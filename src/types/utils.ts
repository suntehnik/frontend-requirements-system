...
import type { EntityType, Priority, EpicStatus, UserStoryStatus, RequirementStatus } from './common'

// Union type for all entities
export type Entity = Epic | UserStory | AcceptanceCriteria | Requirement

// Utility type to extract entity by type
export type EntityByType<T extends EntityType> = T extends 'epic'
  ? Epic
  : T extends 'user_story'
    ? UserStory
    : T extends 'acceptance_criteria'
      ? AcceptanceCriteria
...
export type StatusByEntityType<T extends EntityType> = T extends 'epic'
  ? EpicStatus
  : T extends 'user_story'
    ? UserStoryStatus
    : T extends 'requirement'
...
export type IncludeParam<T extends string> =
  | T
  | `${T},${string}`
  | `${string},${T}`
  | `${string},${T},${string}`
...
export type UserStoryInclude = IncludeParam<
  'epic' | 'creator' | 'assignee' | 'acceptance_criteria' | 'requirements' | 'comments'
>
export type AcceptanceCriteriaInclude = IncludeParam<
  'user_story' | 'author' | 'requirements' | 'comments'
>
export type RequirementInclude = IncludeParam<
  | 'user_story'
  | 'acceptance_criteria'
  | 'type'
  | 'creator'
