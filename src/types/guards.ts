...
export function isRequirementStatus(status: unknown): status is RequirementStatus {
  return typeof status === 'string' && ['Draft', 'Active', 'Obsolete'].includes(status)
}
...
export function isUserStory(obj: unknown): obj is UserStory {
  if (!hasProperties(obj, ['id', 'reference_id', 'title', 'status', 'priority', 'epic_id']))
    return false
...
export function isComment(obj: unknown): obj is Comment {
  if (
    !hasProperties(obj, ['id', 'content', 'entity_type', 'entity_id', 'author_id', 'is_resolved'])
  )
    return false
