...
export class ConfigService extends BaseApiService {
  ...
  // Requirement Types
  async createRequirementType(data: { name: string; description?: string }): Promise<RequirementType> {
  ...
  async updateRequirementType(id: string, data: { name?: string; description?: string }): Promise<RequirementType> {
  ...
  // Relationship Types
  async createRelationshipType(data: { name: string; description?: string }): Promise<RelationshipType> {
  ...
  async updateRelationshipType(id: string, data: { name?: string; description?: string }): Promise<RelationshipType> {
  ...
  async updateStatusModel(id: string, data: {
    name?: string;
    description?: string;
    is_default?: boolean;
  }): Promise<StatusModel> {
  ...
  async getStatusesByModel(modelId: string): Promise<StatusListResponse> {
    return await this.apiGet<StatusListResponse>(`${this.configPath}/status-models/${modelId}/statuses`);
  }

  async getTransitionsByModel(modelId: string): Promise<StatusTransitionListResponse> {
    return await this.apiGet<StatusTransitionListResponse>(`${this.configPath}/status-models/${modelId}/transitions`);
  }
  ...
  async updateStatusTransition(id: string, data: {
    name?: string;
    description?: string;
  }): Promise<StatusTransition> {
