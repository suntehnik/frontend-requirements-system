...
export class CommentService extends BaseApiService {
  ...
  async createEntityComment(entityType: EntityType, entityId: string, request: CreateCommentRequest): Promise<Comment> {
    const entityPath = this.getEntityPath(entityType);
    return await this.apiPost<Comment>(`/${entityPath}/${entityId}/comments`, request);
  }

  // Inline comment operations
  async createInlineComment(entityType: EntityType, entityId: string, request: CreateInlineCommentRequest): Promise<Comment> {
    const entityPath = this.getEntityPath(entityType);
