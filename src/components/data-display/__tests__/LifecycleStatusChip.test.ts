...
import LifecycleStatusChip from '../LifecycleStatusChip.vue'
...
describe('LifecycleStatusChip', () => {
  ...
  const createWrapper = (props = {}) => {
    return mount(LifecycleStatusChip, {
  ...
  describe('Status Color and Text Mapping', () => {
    it('has correct status text mapping', () => {
      ...
      expect(vm.getStatusText('Obsolete')).toBe('Устаревший')
    })
    ...
    it('has correct status color mapping', () => {
      ...
      expect(vm.getStatusColor('Draft')).toBe('grey')
      expect(vm.getStatusColor('Active')).toBe('success')
      expect(vm.getStatusColor('Obsolete')).toBe('warning')
    })
    ...
    it('returns fallback color for unknown status', () => {
      ...
      expect(vm.getStatusColor('UnknownStatus' as LifecycleStatus)).toBe('grey')
    })
    ...
    it('returns fallback text for unknown status', () => {
      ...
      expect(vm.getStatusText('UnknownStatus' as LifecycleStatus)).toBe('UnknownStatus')
    })
  ...
  describe('Event Emission with Proper Typing', () => {
    it('emits status-change event with correct type when status is changed', async () => {
      ...
      expect(wrapper.emitted('status-change')?.[0]).toEqual(['Active'])
    })
    ...
    it('has correct lifecycle status options with proper typing', () => {
      const wrapper = createWrapper()
      const vm = wrapper.vm as any

      expect(vm.lifecycleStatusOptions).toHaveLength(3)
      expect(vm.lifecycleStatusOptions.map((item: any) => item.value)).toEqual([
        'Draft',
        'Active',
        'Obsolete',
      ])
      expect(vm.lifecycleStatusOptions.map((item: any) => item.text)).toEqual([
        'Черновик',
        'Активный',
        'Устаревший',
      ])
  ...
  describe('Error Handling Scenarios', () => {
    ...
    it('validates status values by checking if they exist in status options', () => {
      ...
      const validStatuses = vm.lifecycleStatusOptions.map((option: any) => option.value)
  ...
  describe('Accessibility', () => {
    it('computes correct ARIA label for normal chip', () => {
      ...
      expect(vm.chipAriaLabel).toContain('Статус жизненного цикла: Черновик')
    ...
    it('computes correct ARIA label for readonly chip', () => {
      ...
      expect(vm.chipAriaLabel).toBe('Статус жизненного цикла: Черновик')
    })
    ...
    it('computes correct ARIA label for dropdown', () => {
      ...
      expect(vm.selectAriaLabel).toContain('Выберите статус жизненного цикла')
