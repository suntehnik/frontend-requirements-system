<template>
  <div class="tree-view">
    ...
    <!-- Error State -->
    <v-alert
      v-else-if="hierarchyStore.error"
      type="error"
      variant="tonal"
      class="mb-4"
    >
      ...
      <template #append>
        <v-btn
          size="small"
          variant="text"
          @click="hierarchyStore.loadHierarchy()"
        >
          Повторить
        </v-btn>
      </template>
...
const contextMenuActions = computed(() => {
  ...
  const canComment = authStore.hasRole('Administrator') || authStore.hasRole('User') || authStore.hasRole('Commenter')
  ...
  switch (actionKey) {
    ...
    case 'add-requirement':
      if (node.entity_type === 'user_story') {
        ...
        router.push(`/requirements/new?user_story_id=${userStoryId}&acceptance_criteria_id=${node.entity_id}`)
      }
      break
...
function getParentUserStoryId(node: HierarchyNode): string {
  ...
  const userStory = flattened.find(n => 
